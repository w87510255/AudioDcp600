...
#include <fcntl.h>
  ...
  #include   <windows.h>   
	...
	#include <errno.h>
#endif
//#include "C_LogManage.h"//del by xiaozhengxiu 20140902
#include "C_TcpSocket.h"
#include "C_ErrorDef.h"//add by xiaozhengxiu 20140902
...
//		return iResult;//del by xiaozhengxiu 20140916
		return SOCKET_PARAR_ERR;
	}
...
//20140925 by xiaozhengxiu add
	//说明：建立非阻塞tcp连接。
	//返回值：连接成功返回 0，连接失败 -1；
	//strIp：[in] ，usPort [in], iWait [in];
int C_TcpSocket::TcpConnect_nonb(const char *strIp, unsigned short usPort)
{
	struct sockaddr_in   servaddr;
	int                  flag = 0;
	int iResult = -1;
	...
	int iTmpSocketId = socket(AF_INET, SOCK_STREAM, 0);
	...
	flag = connect_nonb(iTmpSocketId, (struct sockaddr *)&servaddr, sizeof(servaddr), 0);
	if(flag != 0)  //连接失败
	{
#ifdef WIN32
		closesocket(iTmpSocketId); 
	...
	strcpy(m_strIp, strIp);
	m_iSocketId = iTmpSocketId;
	return SOCKET_SUCCEES;
}

//20140925 by xiaozhengxiu add end
...
//20141013 by xiaozhengxiu add
  //临时缓存地址。
  memset(m_TmpBuffer,0,m_iTmpBufferLen);
  //交换数据区
  memset(m_TmpExBuffer,0,m_iTmpExBufferLen);
//20141013 by xiaozhengxiu add end
...
#else
//20141117 by xiaozhengxiu del 发现ssh重启后出现链接失败，注销则正常，原因需待查
...
//del by xiaozhengxiu 20140902 end
		m_strError = str;
		return  0;
}
